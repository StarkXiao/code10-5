...
进入观察期（自补 14 天、送修 7 天）
   ↓
系统到点提醒复检（SSE 推送 + 站内待办），复检按分级判定：
   ├── A 级 良好 → 闭环收口（破损 resolved）
   ├── B 级 尚可 → 收口，或继续观察（30 天后加检）
   ├── C 级 不合格（首轮）→ 返工（生成返工任务，回到修补流程）
   ├── C 级 不合格（连续两轮）→ 自动升级退役评估，并通知衣橱里的家人
   └── 不可修 → 退役（必填处置方式）
（观察期未满就复检需要二次确认，提前复检会在记录上留痕）
   ↓
...
- `tests/graded-review.test.ts`：**分级复检**：动作矩阵（A 闭环 / B 闭环或观察 / C 返工或退役）、观察期未满二次确认、连续两轮 C 级自动升级退役评估并通知家人、中间夹 B 级时连续计数中断
